...
bool CVehicleManager::IsValidModel(unsigned int ulVehicleModel)
{
    if (ulVehicleModel >= 400 && ulVehicleModel <= 611)
        return true;
    if (ulVehicleModel >= CUSTOM_VEHICLE_MIN && ulVehicleModel <= CUSTOM_VEHICLE_MAX)
        return true;
    return false;
}

bool CVehicleManager::IsStandardModel(unsigned int ulVehicleModel)
{
...
eVehicleType CVehicleManager::GetVehicleType(unsigned short usModel)
{
    ...
    // Custom FLA vehicles: look up type from generated data
    std::uint8_t rawType = GetCustomVehicleType(usModel);
    if (rawType != 0xFF)
    {
        switch (rawType)
        {
            case 0:  return VEHICLE_CAR;
            case 1:  return VEHICLE_MONSTERTRUCK;
            case 2:  return VEHICLE_QUADBIKE;
            case 3:  return VEHICLE_HELI;
            case 4:  return VEHICLE_PLANE;
            case 5:  return VEHICLE_BOAT;
            case 6:  return VEHICLE_TRAIN;
            case 9:  return VEHICLE_BIKE;
            case 10: return VEHICLE_BMX;
            case 11: return VEHICLE_TRAILER;
        }
    }
...
unsigned int CVehicleManager::GetMaxPassengers(unsigned int uiVehicleModel)
{
    ...
    // Custom vehicles: use behavior parent's passenger count
    const auto& map = GetCustomBehaviorMap();
    auto it = map.find(uiVehicleModel);
    if (it != map.end() && it->second >= 400 && it->second <= 611)
        return g_ucMaxPassengers[it->second - 400];

    // Default by vehicle type
    eVehicleType type = GetVehicleType(static_cast<unsigned short>(uiVehicleModel));
    switch (type)
    {
        case VEHICLE_BIKE:
        case VEHICLE_BMX:      return 1;
        case VEHICLE_BOAT:     return 0;
        case VEHICLE_QUADBIKE: return 1;
        default:               return 3;
    }
}
...
void CVehicleManager::GetRandomVariation(unsigned short usModel, unsigned char& ucVariant, unsigned char& ucVariant2)
{
    ...
    ucVariant = 255;
    ucVariant2 = 255;
    // Valid stock model? (variants array is only 400-611)
...
// Resolve custom model -> stock parent for attribute lookups
static unsigned int ResolveAttributeModel(unsigned int uiModel)
{
    if (uiModel >= 400 && uiModel <= 611)
        return uiModel;
    const auto& map = GetCustomBehaviorMap();
    auto it = map.find(uiModel);
    if (it != map.end())
        return it->second;
    return 0;
}
...
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_TURRENT));
}

bool CVehicleManager::HasSirens(unsigned int uiModel)
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_SIRENS));
}

bool CVehicleManager::HasTaxiLight(unsigned int uiModel)
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_TAXI_LIGHTS));
}

bool CVehicleManager::HasLandingGears(unsigned int uiModel)
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_LANDING_GEARS));
}

bool CVehicleManager::HasAdjustableProperty(unsigned int uiModel)
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_ADJUSTABLE_PROPERTY));
}

bool CVehicleManager::HasSmokeTrail(unsigned int uiModel)
{
    unsigned int resolved = ResolveAttributeModel(uiModel);
    return (resolved >= 400 && resolved <= 611 && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_SMOKE_TRAIL));
}

bool CVehicleManager::IsTrailer(unsigned int uiVehicleModel)
{
    return (IsValidModel(uiVehicleModel) && GetVehicleType(static_cast<unsigned short>(uiVehicleModel)) == VEHICLE_TRAILER);
}
