...
const CHandlingEntry* CHandlingManager::GetOriginalHandlingData(std::uint32_t model) const noexcept
{
    ...
    // Custom FLA vehicles: use handling data from our generated map
    if (model >= CUSTOM_VEHICLE_MIN && model <= CUSTOM_VEHICLE_MAX)
    {
        static std::unordered_map<std::uint32_t, std::unique_ptr<CHandlingEntry>> s_CustomOriginalEntries;
        ...
        if (it == s_CustomOriginalEntries.end())
        {
            ...
            {
                tHandlingData data = dataIt->second;
                s_CustomOriginalEntries[model] = std::make_unique<CHandlingEntry>(&data);
            }
            else
            {
...
CHandlingEntry* CHandlingManager::GetModelHandlingData(std::uint32_t model) const noexcept
{
    ...
    if (entries == m_ModelEntries.end())
    {
        ...
        if (model >= CUSTOM_VEHICLE_MIN && model <= CUSTOM_VEHICLE_MAX)
        {
            ...
            {
                tHandlingData data = dataIt->second;
                m_ModelEntries[model] = std::make_unique<CHandlingEntry>(&data);
            }
            else
            {
                const eHandlingTypes eHandling = GetHandlingID(model);
...
eHandlingTypes CHandlingManager::GetHandlingID(std::uint32_t model) const noexcept
{
    ...
    if (model >= CUSTOM_VEHICLE_MIN && model <= CUSTOM_VEHICLE_MAX)
    {
        ...
        switch (vtype)
        {
            case 0:  return HT_LANDSTAL;   // car
            case 1:  return HT_MONSTER;     // mtruck
            case 2:  return HT_QUADBIKE;    // quad
            case 3:  return HT_LEVIATHN;    // heli
            case 4:  return HT_SEAPLANE;    // plane
            case 5:  return HT_REEFER;      // boat
            case 6:  return HT_FREIGHT;     // train
            case 9:  return HT_BIKE;        // bike
            case 10: return HT_BMX;         // bmx
            case 11: return HT_ARTICT1;     // trailer
        }
    }
