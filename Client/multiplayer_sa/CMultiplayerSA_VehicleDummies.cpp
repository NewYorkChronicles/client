...
// Read dynamically from FLA-patched instruction operand at 0x403DA4+3.
// FLA may relocate ms_modelInfoPtrs array from 0xA9B0C8 to heap.
// This variable is updated at runtime; asm hooks read it via dword ptr [CModelInfo__ms_modelInfoPtrs].
static uint32_t CModelInfo__ms_modelInfoPtrs = 0xA9B0C8;  // Default; overwritten after FLA init

void InitVehicleDummiesModelInfoPtr()
{
    CModelInfo__ms_modelInfoPtrs = (uint32_t)*(void**)(0x403DA4 + 3);
}
...
{
    // Resolve FLA-relocated ms_modelInfoPtrs before installing hooks that reference it
    InitVehicleDummiesModelInfoPtr();
