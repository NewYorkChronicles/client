...
// Pointer to mod_HandlingManager.m_fWheelFriction, resolved dynamically
// to support FLA (FastLoader Asi) which relocates mod_HandlingManager to heap.
// Stock address is 0xC2B9CC but FLA patches game instructions to use the new address.
static float* s_pfWheelFriction = reinterpret_cast<float*>(0xC2B9CC);
static BYTE   s_savedWheelFrictionInstr[6] = {0xD9, 0x05, 0xCC, 0xB9, 0xC2, 0x00};
...
static void _declspec(naked) HOOK_VehicleRapidStopFix()
{
    ...
    {
        push eax
        mov eax, dword ptr [s_pfWheelFriction]
        fld dword ptr [eax]              // mod_HandlingManager.m_fWheelFriction (dynamic)
        pop eax
        fmul ds:[0xB7CB5C]              // CTimer::ms_fTimeStep
        fdiv kOriginalTimeStep           // 1.666f
...
void CMultiplayerSA::SetRapidVehicleStopFixEnabled(bool enabled)
{
    ...
    if (enabled)
    {
        // Read the (potentially FLA-patched) address of m_fWheelFriction from
        // the game's instruction bytes BEFORE we overwrite them with our hooks.
        // Instruction format: D9 05 [4-byte address] = fld ds:[addr]
        s_pfWheelFriction = *reinterpret_cast<float**>(0x6D6E69 + 2);

        // Save the original 6-byte instruction so we can restore it correctly
        // (FLA may have changed the address from stock 0xC2B9CC)
        MemCpyFast(s_savedWheelFrictionInstr, (const void*)0x6D6E69, 6);
    ...
    else
    {
        ...
        // Restore the original instruction bytes (with correct FLA-relocated address)
        MemCpy((void*)0x6D6E69, s_savedWheelFrictionInstr, 6);
        MemCpy((void*)0x6D6EA8, s_savedWheelFrictionInstr, 6);

        MemCpy((void*)0x6D767F, s_savedWheelFrictionInstr, 6);
        MemCpy((void*)0x6D76AB, s_savedWheelFrictionInstr, 6);
        MemCpy((void*)0x6D76CD, s_savedWheelFrictionInstr, 6);
    }
