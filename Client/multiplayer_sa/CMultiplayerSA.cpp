...
*/
    ...
    */
    ...
    // Disable CPointLights::AddLight — biggest single FPS gain (kills dynamic point lights)
    MemPut<BYTE>(0x7000E0, 0xC3);

    // Freeze vegetation/grass grid bounds to zero — stops per-frame re-streaming
    MemPutFast<float>(0xC7EB20, 0.0f);
    MemPut<DWORD>(0x6E9CA0, 0x00C7EB20);
    MemPut<DWORD>(0x6E9CDB, 0x00C7EB20);
    MemPut<DWORD>(0x6E9D17, 0x00C7EB20);
    MemPut<DWORD>(0x6E9D53, 0x00C7EB20);

    // NOP vehicle PreRender accessory/shadow draw calls
    MemSet((void*)0x6ABCF5, 0x90, 5);
    MemSet((void*)0x6BD667, 0x90, 5);
    MemSet((void*)0x6C0B21, 0x90, 5);
    MemSet((void*)0x6C58A0, 0x90, 5);
    MemSet((void*)0x6CA73A, 0x90, 5);

    // Idle front-end active bypass — skip redundant front-end processing
    MemCpy((void*)0x53E9B3, "\x75\x44\x90\x90\x90\x90", 6);

    // SilentPatch: remove 1-frame render delay in Idle loop
    MemPut<WORD>(0x53E923, 0x43EB);
    MemPut<BYTE>(0x53E99F, 0x10);
    MemSet((void*)0x53E9A5, 0x90, 1);

    // SilentPatch: stop per-frame stencil shadow buffer alloc/free (3x 0x6000 bytes)
    MemCpy((void*)0x711DDA, "\xEB\x2C", 2);
    MemSet((void*)0x711E0D, 0x90, 3);
    MemCpy((void*)0x711E5F, "\x5F\x5D\xC3", 3);

    // SilentPatch: fix CdStream thread sync — prevent busy-wait spinning
    MemPut<BYTE>(0x406910, 0x00);
...
const DWORD RETURN_CCollision__CheckCameraCollisionObjects_3 = 0x41ABA5; // past model info + pColModel + boundingSphere derefs
...
void _declspec(naked) HOOK_CCollision__CheckCameraCollisionObjects()
{
    ...
    out1:
        // Validate ms_modelInfoPtrs[modelIndex], pColModel, and boundingSphere.radius
        // before continuing. Game code at 0x41AB98-0x41ABA2 does:
        //   mov eax, ms_modelInfoPtrs[edx*4]   ; model info
        //   mov ecx, [eax+14h]                  ; pColModel
        //   mov edx, [ecx+24h]                  ; boundingSphere.radius
        // With FLA any of these can be NULL or garbage.
        // We inline this sequence with validation, then jump past it to 0x41ABA5.
        mov     eax, ds:[0x403DA7]          // FLA-patched ms_modelInfoPtrs base
        mov     eax, [eax+edx*4]            // ms_modelInfoPtrs[modelIndex]
        test    eax, eax
        jz      out2
        cmp     eax, 0x10000
        jb      out2
        ...
        cmp     ecx, 0x10000
        jb      out2
        mov     edx, [ecx+24h]              // boundingSphere.radius (float as int)
        jmp     RETURN_CCollision__CheckCameraCollisionObjects_3  // 0x41ABA5 — past all derefs
