...
void _declspec(naked) HOOK_CrashFix_Misc5()
{
    ...
    __asm
    {
        mov edi, ds:[0x403DA7]          // Read FLA-patched ms_modelInfoPtrs base
...
void _declspec(naked) HOOK_CAnimManager_CreateAnimAssocGroups()
{
    ...
    __asm
    {
        ...
        // Replaced code — read ms_modelInfoPtrs from FLA-patched operand
        push    ecx
        mov     ecx, ds:[0x403DA7]
...
static void _declspec(naked) HOOK_CWorld__FindObjectsKindaCollidingSectorList()
{
    ...
    __asm
    {
        // Read ms_modelInfoPtrs base from FLA-patched instruction operand (0x403DA4+3)
        // instead of hardcoding 0xA9B0C8 which FLA may have relocated.
        mov eax, ds:[0x403DA7]      // *(void**)(0x403DA4+3) = relocated ms_modelInfoPtrs base
        mov eax, [eax+edx*4]        // index into the (possibly relocated) array
        test eax, eax
        jz skip
...
////////////////////////////////////////////////////////////////////////
// CWorld::ProcessLineOfSightSectorList — ColModel pointer validation
//
// FLA expands model info arrays; entries can be garbage (non-NULL but
// invalid). AnimatePedColModelSkinned / GetColModel may return an
// invalid ColModel pointer that passes the existing NULL check.
// Validate the pointer before dereferencing it.
////////////////////////////////////////////////////////////////////////
#define HOOKPOS_CrashFix_ProcessLOS_ColModel  0x5670BC
#define HOOKSIZE_CrashFix_ProcessLOS_ColModel 10
DWORD                 RETURN_CrashFix_ProcessLOS_ColModel = 0x5670C6;
DWORD                 RETURN_CrashFix_ProcessLOS_ColModel_Skip = 0x56727E;
void _declspec(naked) HOOK_CrashFix_ProcessLOS_ColModel()
{
    // clang-format off
    __asm
    {
        // eax = ColModel pointer returned from AnimatePedColModelSkinned or GetColModel
        // ebp = 0 at this point
        mov     esi, eax
        test    esi, esi
        jz      skip
        cmp     esi, 0x10000        // reject low-address garbage pointers
        jb      skip
        jmp     RETURN_CrashFix_ProcessLOS_ColModel     // 0x5670C6 — continue normally
    skip:
        jmp     RETURN_CrashFix_ProcessLOS_ColModel_Skip // 0x56727E — skip this entity
    }
    // clang-format on
}
...
void CMultiplayerSA::InitHooks_CrashFixHacks()
{
    ...
    EZHookInstall(CrashFix_ProcessLOS_ColModel);
