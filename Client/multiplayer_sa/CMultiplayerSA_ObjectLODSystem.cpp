...
DWORD                 RETURN_CRenderer_SetupEntityVisibility_AfterModelInfo = 0x55424B;
DWORD                 RETURN_CRenderer_SetupEntityVisibility_Skip = 0x55441C; // return 0 (entity not visible)
...
second:
        ...
        // Original: 0x554238 - movsx eax, word ptr [esi+22h]
        movsx   eax, word ptr [esi+22h]
        // Original: 0x55423C - push edi
        push    edi
        // Original: 0x55423D - mov edi, ds:ms_modelInfoPtrs[eax*4]
        // FLA relocates ms_modelInfoPtrs — read the base from FLA-patched operand
        mov     edi, ds:[0x403DA7]          // FLA-patched ms_modelInfoPtrs base pointer
        mov     edi, [edi+eax*4]            // edi = ms_modelInfoPtrs[modelIndex]
        // Null-check: FLA expands model ID range, many slots are NULL
        test    edi, edi
        jz      skip_entity
        // Original: 0x554244 - mov edx, [edi]  (vtable)
        mov     edx, [edi]
        // Original: 0x554246 - mov ecx, edi
        mov     ecx, edi
        // Original: 0x554248 - call dword ptr [edx+4]
        call    dword ptr [edx+4]
        jmp     RETURN_CRenderer_SetupEntityVisibility_AfterModelInfo  // 0x55424B
