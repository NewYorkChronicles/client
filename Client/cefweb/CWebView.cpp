...
void CWebView::Initialise()
{
    ...
    // Cap frame rate - no point rendering browser faster than the game
    int frameLimit = g_pCore->GetFrameRateLimit();
    if (frameLimit <= 0 || frameLimit > 60)
        frameLimit = 60;            // Cap CEF at 60fps max even if game runs higher
    browserSettings.windowless_frame_rate = frameLimit;
    ...
    // Disable WebGL by default - game UI scripts rarely need it, saves GPU memory
    browserSettings.webgl = cef_state_t::STATE_DISABLED;

    // Disable features not needed for game UI
    browserSettings.local_storage = cef_state_t::STATE_ENABLED;
    browserSettings.tab_to_links = cef_state_t::STATE_DISABLED;
    browserSettings.text_area_resize = cef_state_t::STATE_DISABLED;
    ...
    if (!m_bIsLocal)
    {
        ...
        // Remote browsers may need WebGL and storage
...
void CWebView::SetRenderingPaused(bool bPaused)
{
    if (m_pWebView)
    {
        ...
        if (bPaused)
        {
            ...
            m_RenderData.hasPendingUpdate.store(false, std::memory_order_release);
...
{
    if (!m_RenderData.hasPendingUpdate.load(std::memory_order_acquire))
        return;
...
void CWebView::OnPopupShow(CefRefPtr<CefBrowser> browser, bool show)
{
    ...
    if (show)
        m_RenderData.hasPendingUpdate.store(true, std::memory_order_release);
