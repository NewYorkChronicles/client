...
{
    // Allocate with extra padding for FLA compatibility.
    // FLA patches ConvertDataToGameUnits to call IsBikeHandling()/IsRCbandit() which
    // read tHandlingData_extended fields (pBikeHandling, handlingName) past the stock
    // struct size. Zero-filling the padding ensures those reads return null/false.
    constexpr size_t kAllocSize = sizeof(tHandlingDataSA) + FLA_EXTENDED_PADDING;
    auto* pMem = static_cast<tHandlingDataSA*>(::operator new(kAllocSize));
    MemSetFast(pMem, 0, kAllocSize);
    m_HandlingSA.reset(pMem);
}
