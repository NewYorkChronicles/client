...
// Initialized to nullptr — resolved lazily or in CGameSA constructor after FLA has patched.
// FLA relocates ms_modelInfoPtrs to heap; the address is read from the patched operand at 0x403DA4+3.
CBaseModelInfoSAInterface** CModelInfoSAInterface::ms_modelInfoPtrs = nullptr;
CBaseModelInfoSAInterface** ppModelInfo = nullptr;
...
// FLA may relocate COL base ID — read dynamically from the patched instruction operand.
// Vanilla: 25000 (20000 DFF + 5000 TXD). FLA: 39000 (29000 DFF + 10000 TXD).
static size_t GetResourceIdCol() { return static_cast<size_t>(*(int32_t*)(0x410344 + 2)); }
...
bool CModelInfoSA::IsVehicle() const
{
    ...
    if (m_dwModelID >= static_cast<DWORD>(pGame->GetBaseIDforTXD()))
        return false;
...
{
    DWORD dwModelInfoArray = (DWORD)ARRAY_ModelInfo;
    ...
    // clang-format off
        __asm
        {
            ...
            mov     ebx, ModelID
            lea     ebx, [ebx*4]
            add     ebx, dword ptr[dwModelInfoArray]
            mov     eax, [ebx]
            add     eax, 50
...
void CModelInfoSA::Request(EModelRequestType requestType, const char* szTag)
{
    ...
    if (requestType == BLOCKING)
    {
        ...
        {
            int iCount = 0;
            DWORD dwLastNumRequested = *(DWORD*)0x08E4CB8;
            int iNoProgressCount = 0;
            while (iCount++ < 10 && !IsLoaded())
            {
                ...
                const DWORD dwNumRequested = *(DWORD*)0x08E4CB8;
                if (dwNumRequested == dwLastNumRequested)
                {
                    if (++iNoProgressCount >= 2)
                        break;
                }
                else
                {
                ...
                dwLastNumRequested = dwNumRequested;
            }
            if (!IsLoaded())
            {
...
float CModelInfoSA::GetDistanceFromCentreOfMassToBaseOfModel()
{
    ...
    __asm
    {
        ...
        push    ecx
        mov     ecx, dword ptr[dwModelInfoArray]
...
{
    // IPLs should not contain peds, weapons, vehicles and vehicle upgrades (including custom FLA vehicles)
    if (m_dwModelID > 611 && (m_dwModelID < 1000 || m_dwModelID > 1193) && (m_dwModelID < CUSTOM_VEHICLE_MIN || m_dwModelID > CUSTOM_VEHICLE_MAX))
    {
...
void CModelInfoSA::SetCustomCarPlateText(const char* szText)
{
    ...
    __asm
    {
        ...
        push    eax
        mov     eax, dword ptr[dwModelInfoArray]
...
void* CModelInfoSA::GetVehicleSuspensionData()
{
    ...
    if (!pColData)
    {
        ...
        const DWORD colId = static_cast<DWORD>(GetResourceIdCol() + slot);
...
void CModelInfoSA::RestoreColModel()
{
    ...
    if (m_pInterface && m_pOriginalColModelInterface && m_pCustomColModel)
    {
        ...
        if (m_pInterface->pColModel)
        {
            ...
            {
                if (SafeReadColSlot(m_pInterface->pColModel, &slot))
                    pGame->GetStreaming()->RemoveModel(GetResourceIdCol() + slot);
                else
...
void CModelInfoSA::AddColRef()
{
    ...
    // FLA supports collision slots beyond 255, removed the slot > 0xFF check
    if (slot == 0xFFFF)
        return;
...
void CModelInfoSA::DeallocateModel()
{
    ...
    if (!IsValidModelInfoPtr(pInterfaceToDelete))
    {
        ...
        m_pCustomColModel = nullptr;
        m_pOriginalColModelInterface = nullptr;
        // FLA supports collision slots beyond 255, removed the m_usColSlot <= 0xFF check
