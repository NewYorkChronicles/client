...
using tHandlingData = tHandlingDataSA;
#define CUSTOM_VEHICLE_HANDLING_AVAILABLE
...
{
    // Custom FLA vehicles: use handling data from our generated map (check BEFORE IsVehicleModel
    // because FLA model info may not be fully initialized when the vehicle constructor runs)
    if (model >= CUSTOM_VEHICLE_MIN && model <= CUSTOM_VEHICLE_MAX)
    {
        static std::unordered_map<std::uint32_t, std::unique_ptr<CHandlingEntrySA>> s_CustomOriginalEntries;
        auto it = s_CustomOriginalEntries.find(model);
        if (it == s_CustomOriginalEntries.end())
        {
            const auto& handlingMap = GetCustomHandlingDataMap();
            auto dataIt = handlingMap.find(model);
            if (dataIt != handlingMap.end())
            {
                tHandlingDataSA data = dataIt->second;
                s_CustomOriginalEntries[model] = std::make_unique<CHandlingEntrySA>(&data);
            }
            else
            {
                // No custom data found, fall back to type-based default
                const HandlingType eHandling = GetHandlingID(model);
                return m_OriginalEntries[(std::size_t)eHandling].get();
            }
            it = s_CustomOriginalEntries.find(model);
        }
...
HandlingType CHandlingManagerSA::GetHandlingID(std::uint32_t model) noexcept
{
    ...
    // Custom FLA vehicles: return type-based default (FLA loads actual handling from handling.cfg)
    if (model >= CUSTOM_VEHICLE_MIN && model <= CUSTOM_VEHICLE_MAX)
    {
        std::uint8_t vtype = GetCustomVehicleType(static_cast<std::uint32_t>(model));
        switch (vtype)
        {
            case 0:  return HandlingType::HT_LANDSTAL;   // car
            case 1:  return HandlingType::HT_MONSTER;     // mtruck
            case 2:  return HandlingType::HT_QUADBIKE;    // quad
            case 3:  return HandlingType::HT_LEVIATHN;    // heli
            case 4:  return HandlingType::HT_SEAPLANE;    // plane
            case 5:  return HandlingType::HT_REEFER;      // boat
            case 6:  return HandlingType::HT_FREIGHT;     // train
            case 9:  return HandlingType::HT_BIKE;        // bike
            case 10: return HandlingType::HT_BMX;         // bmx
            case 11: return HandlingType::HT_ARTICT1;     // trailer
        }
    }
