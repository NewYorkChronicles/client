...
// FLA compatibility: ConvertDataToGameUnits (patched by FLA at runtime) calls
// IsBikeHandling() and IsRCbandit() which read tHandlingData_extended fields
// (pBikeHandling, handlingName, etc.) past sizeof(tHandlingDataSA). MTA's own
// handling buffers must be padded so those reads hit zeroes instead of heap garbage.
// Without this, IsBikeHandling() returns true for cars → bike reverse speed (-0.1).
static constexpr size_t FLA_EXTENDED_PADDING = 64;
...
private:
    // Custom deleter for over-sized allocation (sizeof(tHandlingDataSA) + FLA_EXTENDED_PADDING)
    struct HandlingDeleter { void operator()(tHandlingDataSA* p) const noexcept { ::operator delete(p); } };
    std::unique_ptr<tHandlingDataSA, HandlingDeleter> m_HandlingSA;
