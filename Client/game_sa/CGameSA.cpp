...
CGameSA::CGameSA()
{
    try
    {
        ...
        // Resolve FLA-relocated pointers. FLA (fastman92 limit adjuster) patches instruction operands
        // to point to heap-allocated arrays. We read those operands now (after FLA has loaded) to get
        // the correct (possibly relocated) addresses.
        CModelInfoSAInterface::ms_modelInfoPtrs = (CBaseModelInfoSAInterface**)GetARRAY_ModelInfo();
        extern CBaseModelInfoSAInterface** ppModelInfo;
        ppModelInfo = CModelInfoSAInterface::ms_modelInfoPtrs;
        ...
        // Use std::max with current value so FLA's higher limits are not downgraded.
        auto SetPoolMax = [this](ePools pool, int mtaValue) {
            int current = m_Pools->GetPoolCapacity(pool);
            m_Pools->SetPoolCapacity(pool, std::max(current, mtaValue));
        };
        SetPoolMax(TASK_POOL, 5000);                                     // Default is 500
        SetPoolMax(OBJECT_POOL, MAX_OBJECTS);                            // Default is 350
        SetPoolMax(EVENT_POOL, 5000);                                    // Default is 200
        SetPoolMax(COL_MODEL_POOL, 12000);                              // Default is 10150
        SetPoolMax(ENV_MAP_MATERIAL_POOL, 16000);                       // Default is 4096
        SetPoolMax(ENV_MAP_ATOMIC_POOL, 4000);                          // Default is 1024
        SetPoolMax(SPEC_MAP_MATERIAL_POOL, 16000);                      // Default is 4096
        SetPoolMax(ENTRY_INFO_NODE_POOL, MAX_ENTRY_INFO_NODES);         // Default is 500
        SetPoolMax(POINTER_SINGLE_LINK_POOL, MAX_POINTER_SINGLE_LINKS); // Default is 70000
        SetPoolMax(POINTER_DOUBLE_LINK_POOL, MAX_POINTER_DOUBLE_LINKS); // Default is 3200
