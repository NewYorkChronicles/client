...
CObject* CPoolsSA::AddObject(CClientObject* pClientObject, DWORD dwModelID, bool bLowLod, bool bBreakingDisabled)
{
    ...
    if (m_objectPool.ulCount < MAX_OBJECTS)
    {
        ...
        if (!pModelInfo->GetInterface()->pColModel)
        {
            AddReportLog(5553, SString("Failed to create object with model %d - collision model not loaded", dwModelID));
            return nullptr;
        }
...
{
    // Return the actual current capacity from the patched operands.
    // FLA and MTA both write pool sizes to these operands, and CGameSA::Init
    // uses std::max to preserve whichever is larger.
    return GetPoolCapacity(pool);
}
