...
CBuildingsPoolSA::CBuildingsPoolSA() : m_pOriginalBuildingsBackup(nullptr)
{
    ...
    // FLA may expand the building pool beyond MAX_BUILDINGS (13000 vanilla → 30000 FLA).
    // Resize our tracking vector to match the actual pool size.
    if (*m_ppBuildingPoolInterface)
    {
        int actualSize = (*m_ppBuildingPoolInterface)->m_nSize;
        if (actualSize > 0 && static_cast<size_t>(actualSize) > m_buildingPool.entities.size())
            m_buildingPool.entities.resize(actualSize, {nullptr, nullptr});
    }
}
...
void CBuildingsPoolSA::RemoveAllWithBackup()
{
    ...
    m_pOriginalBuildingsBackup = std::make_unique<backup_array_t>(poolSize);
...
void CBuildingsPoolSA::RestoreBackup()
{
    ...
    auto  pBuildsingsPool = (*m_ppBuildingPoolInterface);
    const size_t poolSize = originalData.size();
    for (size_t i = 0; i < poolSize; i++)
    {
...
void CBuildingsPoolSA::UpdateBackupLodPointers(uint32_t offset)
{
    ...
    const size_t poolSize = arr->size();
    for (size_t i = 0; i < poolSize; ++i)
    {
