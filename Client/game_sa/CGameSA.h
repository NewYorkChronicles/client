...
// FLA may change the streaming ID layout (DFF/TXD/COL/IPL counts).
// Read limits dynamically from FLA-patched instruction operands.
// These read the same operands as GetBaseIDforTXD/COL/GetCountOfAllFileIDs
// but avoid circular dependency with CGameSA class definition.
inline uint32_t GetModelInfoDffMax()
{
    // Base ID for TXD = DFF count. FLA patches operand at 0x407104+2.
    return static_cast<uint32_t>(*(int32_t*)(0x407104 + 2));
}
inline uint32_t GetModelInfoTxdMax()
{
    // Base ID for COL = DFF+TXD count. FLA patches operand at 0x410344+2.
    return static_cast<uint32_t>(*(int32_t*)(0x410344 + 2));
}
inline uint32_t GetModelInfoMax()
{
    // Total file IDs = (end_ptr - start_ptr) / sizeof(CStreamingInfo)
    // FLA patches both operands at 0x5B8AFA+2 and 0x5B8B08+6.
    return static_cast<uint32_t>((*(char**)(0x5B8AFA + 2) - *(char**)(0x5B8B08 + 6)) / 20);
}
#define MODELINFO_DFF_MAX GetModelInfoDffMax()
#define MODELINFO_TXD_MAX GetModelInfoTxdMax()
#define MODELINFO_MAX     GetModelInfoMax()
