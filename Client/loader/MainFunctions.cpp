...
void CheckLibVersions()
{
    ...
    for (uint i = 0; i < NUMELMS(moduleList); i++)
    {
        ...
        if (FileExists(fullPath))
        {
            ...
            {
                DisplayErrorMessageBox(SStringX(_("File version mismatch error. Reinstall New York Chronicles if you experience problems.\n") +
        ...
        {
            DisplayErrorMessageBox(SStringX(_("Some files are missing. Reinstall New York Chronicles if you experience problems.\n") + SString("\n[%s]\n", *strFilename)),
    ...
    {
        DisplayErrorMessageBox(SStringX(_("New York Chronicles is not compatible with Windows 'Safe Mode'.\n\nPlease restart your PC.\n")), _E("CL42"), "safe-mode");
        ExitProcess(EXIT_ERROR);
    }
}
...
void InitLocalization(bool bShowErrors)
{
    ...
    if (!pFunc)
    {
        ...
        if (bShowErrors)
        {
            DisplayErrorMessageBox("Loading localization failed. Please ensure that the locale folder is accessible.", _E("CL26"),
...
void HandleTrouble()
{
    ...
    int iResponse = MessageBoxUTF8(NULL, _("Are you having problems running New York Chronicles?.\n\nDo you want to revert to an earlier version?"),
                                   "New York Chronicles" + _E("CL07"), MB_YESNO | MB_ICONQUESTION | MB_TOPMOST);
    if (iResponse == IDYES)
    {
...
void HandleResetSettings()
{
    ...
    if (FileExists(strSettingsFilename))
    {
        int iResponse = MessageBoxUTF8(NULL,
                                       _("There seems to be a problem launching New York Chronicles.\nResetting GTA settings can sometimes fix this problem.\n\nDo you want "
                                         "to reset GTA settings now?"),
                                       "New York Chronicles" + _E("CL08"), MB_YESNO | MB_ICONQUESTION | MB_TOPMOST);
        if (iResponse == IDYES)
        {
            ...
            if (!FileExists(strSettingsFilename))
            {
                ...
                MessageBoxUTF8(NULL, _("GTA settings have been reset.\n\nPress OK to continue."), "New York Chronicles", MB_OK | MB_ICONINFORMATION | MB_TOPMOST);
            }
            else
            {
    ...
    else
    {
        ...
        int iResponse = MessageBoxUTF8(NULL, _("Are you having problems running New York Chronicles?.\n\nDo you want to see some online help?"), "New York Chronicles",
...
void HandleNotUsedMainMenu()
{
    ...
    // Check current display mode in coreconfig.xml
    {
        ...
        else if (!strWindowed.empty() && !strFullscreenStyle.empty())
        {
            ...
            {
                // Inform user
                SString strMessage = _("Are you having problems running New York Chronicles?.\n\nDo you want to change the following setting?");
                strMessage += "\n" + _("Fullscreen mode:") + " -> " + _("Borderless window");
                HideSplash();
                int iResponse = MessageBoxUTF8(NULL, strMessage, "New York Chronicles", MB_YESNO | MB_ICONQUESTION | MB_TOPMOST);
                if (iResponse == IDYES)
                {
...
void ConfigureWerDumpPath()
{
    ...
    // Get loader's own path directly - this is the install root where game.exe lives
    ...
    *lastSlash = L'\0';
    ...
    // Always use registry fallback for gta_sa.exe (child process), and for game.exe
    ...
    const wchar_t* exeNames[] = {L"gta_sa.exe", loaderConfigured ? nullptr : L"game.exe"};
...
void HandleIfGTAIsAlreadyRunning()
{
    ...
    {
        if (MessageBoxUTF8(
                0, _("An instance of GTA: San Andreas is already running. It needs to be terminated before New York Chronicles can be started. Do you want to do that now?"),
...
//
// Handle duplicate launching, or running from nyc:// URI
//
//////////////////////////////////////////////////////////
void HandleDuplicateLaunching()
{
    ...
    while (!bIsCrashDialog && !CreateSingleInstanceMutex())
    {
        if (cmdLineLen > 0)
        {
            ...
            HWND hwMTAWindow = FindWindow(NULL, "New York Chronicles");
#ifdef MTA_DEBUG
            if (!hwMTAWindow)
                hwMTAWindow = FindWindow(NULL, "New York Chronicles [DEBUG]");
#endif
                ...
                if (aCommandLineArgs && numArgs > 0 && numArgs < 1000)
                {
                    for (int i = 1; i < numArgs; ++i)
                    {
                        ...
                        if (wideArg.length() > 8 && wideArg.length() < 2048 &&  // Max MTA connect URI length
                            WStringX(wideArg).BeginsWith(L"nyc://"))
                        {
                            ...
                            // Additional validation for nyc:// URI content
            ...
            else
            {
                const SString strMessage =
                    _("Trouble restarting New York Chronicles\n\n"
                      "If the problem persists, open Task Manager and\n"
                      "stop the 'gta_sa.exe' and 'game.exe' processes\n\n\n"
                      "Try to launch New York Chronicles again?");
...
void ValidateGTAPath()
{
    ...
    {
        DisplayErrorMessageBox(_("GTA: San Andreas was not found in the 'gta' folder. Please ensure GTA:SA is installed in the 'gta' subfolder next to game.exe."), _E("CL12"), "gta-path-missing");
        ExitProcess(EXIT_ERROR);
    }
    else if (result == GAME_PATH_UNICODE_CHARS)
    {
        DisplayErrorMessageBox(_("The path to your installation of GTA: San Andreas contains unsupported (unicode) characters. "
                                 ...
                                 "only standard ASCII characters and reinstall New York Chronicles."),
                               _E("CL13"));
        ExitProcess(EXIT_ERROR);
    }
    ...
    {
        DisplayErrorMessageBox(_("The path to your installation of 'New York Chronicles' or 'GTA: San Andreas'\n"
                                 "contains a ';' (semicolon).\n\n"
                                 "If you experience problems when running New York Chronicles,\n"
...
void CheckDataFiles()
{
    ...
    static const char* dataFiles[] = {"MTA\\cgui\\images\\radarset\\up.png",
...
int LaunchGame(SString strCmdLine)
{
    ...
    {
        sanitizedCmdLine = SString("nyc://%s", *strPostUpdateConnect);
    }
