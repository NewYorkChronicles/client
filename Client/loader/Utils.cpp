...
{
    // GTA:SA must be in a "gta" subfolder next to game.exe
    SString strGtaPath = PathJoin(GetLaunchPath(), "gta");

    WriteDebugEvent(SString("GetGamePath: Checking hardcoded path '%s'", strGtaPath.c_str()));

    SString strCheckPath = PathJoin(strGtaPath, MTA_GTA_KNOWN_FILE_NAME);
    if (FileExists(strCheckPath))
    {
        strOutResult = strGtaPath;
        WriteDebugEvent(SString("GetGamePath: Found GTA at '%s'", strOutResult.c_str()));
        return GAME_PATH_OK;
    }

    AddReportLog(3201, SString("GetGamePath: GTA not found at '%s'", strCheckPath.c_str()));
...
void BsodDetectionPreLaunch()
{
    ...
    if (strMinidumpTime > strLastMinidumpTime)
    {
        ...
        if (strMinidumpTime > strGameBeginTime && !strGameBeginTime.empty() && iBsodDetectSkip == 0)
        {
            // Ask user to confirm
            int iResponse =
                MessageBoxUTF8(NULL, _("Did your computer restart when playing New York Chronicles?"), "New York Chronicles", MB_YESNO | MB_ICONQUESTION | MB_TOPMOST);
            if (iResponse == IDYES)
            {
...
WerCrashInfo QueryWerCrashInfo(DWORD targetExceptionCode)
{
    ...
    const char* werArchivePath = "C:\\ProgramData\\Microsoft\\Windows\\WER\\ReportArchive";
    ...
    for (auto it = reportDirs.rbegin(); it != reportDirs.rend(); ++it)
    {
        const SString& reportDir = *it;
