...
SString CInstallManager::_ChangeToAdmin()
{
    ...
    {
        MessageBoxUTF8(
            NULL, SString(_("New York Chronicles needs Administrator access for the following task:\n\n  '%s'\n\nPlease confirm in the next window."), *m_strAdminReason),
        ...
        CreateSingleInstanceMutex();
        ClearIsBlockingUserProcess();
        MessageBoxUTF8(NULL, SString(_("New York Chronicles could not complete the following task:\n\n  '%s'\n"), *m_strAdminReason),
                       "New York Chronicles" + _E("CL01"), MB_OK | MB_ICONWARNING | MB_TOPMOST);
    }
    return "fail";
}
...
SString CInstallManager::_InstallFiles()
{
    ...
    if (!InstallFiles(m_pSequencer->GetVariable(HIDE_PROGRESS) != "yes"))
    {
        ...
        m_strAdminReason = _("Install updated New York Chronicles files");
        return "fail";
    }
    else
    {
...
{
    MessageBoxUTF8(0, SString(_("New York Chronicles has not been installed properly, please reinstall. %s"), *strExtraInfo), _("Error") + _E("CL03"),
    ...
    for (const char* fileName : {LOADER_PROXY_DLL_NAME})
    {
        ...
        if (std::error_code ec; !SafeCopyFile(sourcePath, targetPath, ec))
        {
            ...
            if (isAdmin)
            {
                ...
                {
                    SString strMessage(_("New York Chronicles cannot launch because copying a file failed:"));
                    strMessage += "\n\n" + targetPath.u8string();
                    BrowseToSolution("copy-files", ASK_GO_ONLINE, strMessage);
                }
                else
                {
                    SString strMessage(_("New York Chronicles cannot launch because a New York Chronicles file is incorrect or missing:"));
                    strMessage += "\n\n" + sourcePath.u8string();
            ...
            {
                m_strAdminReason = _("Copy New York Chronicles files");
                return "fail";
            }
        }
    }
...
SString CInstallManager::_ProcessGtaPatchCheck()
{
    ...
    {
        SString strMessage(_("New York Chronicles cannot launch because a GTA:SA file is incorrect or missing:"));
...
SString CInstallManager::_ProcessGtaVersionCheck()
{
    ...
    if (!patcher.Load(ec))
    {
        ...
        if (!gtasa.GenerateFile("data", gtaExePath, ec))
        {
            ...
            if (isAdmin)
            {
                SString strMessage(_("New York Chronicles cannot launch because the GTA:SA executable is incorrect or missing:"));
                strMessage += "\n\n" + gtaExePath.u8string();
                strMessage +=
                    "\n\n" +
                    _("Please check your anti-virus for a false-positive detection, try to add an exception for the GTA:SA executable and restart New York Chronicles.");
        ...
        if (!patcher.Load(ec))
        {
            ...
            if (isAdmin)
            {
                SString strMessage(_("New York Chronicles cannot launch because the GTA:SA executable is not loadable:"));
                strMessage += "\n\n" + gtaExePath.u8string();
                BrowseToSolution(SString("gengta_error&code=%d", ec.value()), ASK_GO_ONLINE, strMessage);
                return "quit";
            }
    ...
    if (!patcher.ApplyPatches(ec))
    {
        ...
        if (isAdmin)
        {
            SString strMessage(_("New York Chronicles cannot launch because patching GTA:SA has failed:"));
            strMessage += "\n\n" + gtaExePath.u8string();
...
SString CInstallManager::_ProcessLayoutChecks()
{
    ...
    {
        SString strMessage(_("New York Chronicles cannot continue because drive %s does not have enough space."), *strDriveWithNoSpace.Left(1));
...
SString CInstallManager::_ProcessLangFileChecks()
{
    ...
    if (strSrcFile.empty())
    {
        ...
        strMessage += "\n\n";
        strMessage += _("If the game fails to load, please re-install GTA:SA");
        MessageBoxUTF8(NULL, strMessage, "New York Chronicles", MB_OK | MB_ICONWARNING | MB_TOPMOST);
        return "ok";
    }
