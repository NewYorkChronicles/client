...
namespace
{
    ...
    inline constexpr std::string_view  CRASH_MSG_DIALOG_FAIL = "New York Chronicles has crashed. (Failed to create crash dialog)";
    inline constexpr std::string_view  CRASH_TITLE_FATAL = "New York Chronicles - Fatal Error";
...
const SDialogItemInfo g_CrashedDialogItems[] = {
    {0, 0, _td("New York Chronicles has encountered a problem")},
    {IDC_CRASH_HEAD, 0, _td("Crash information")},
    {IDC_SEND_DUMP_CHECK, 0, _td("Tick the check box to send this crash info to the devs using the 'internet'")},
    {IDC_SEND_DESC_STATIC, 0, _td("Doing so will increase the chance of this crash being fixed.")},
    {IDC_RESTART_QUESTION_STATIC, 1, _td("Do you want to restart New York Chronicles?")},
...
const SDialogItemInfo g_GraphicsDllDialogItems[] = {
    {0, 0, _td("New York Chronicles - Warning")},
    {IDC_D3DDLL_TEXT1, 0, _td("Your Grand Theft Auto: San Andreas install directory contains these files:")},
    {IDC_D3DDLL_TEXT2, 0,
     _td("These files are not required and may interfere with the graphical features in this version of New York Chronicles.\n\n"
...
const SDialogItemInfo g_OptimusDialogItems[] = {
    {0, 0, _td("New York Chronicles - Display options")},
...
[[nodiscard]] SString ShowCrashedDialog(HINSTANCE hInstance, const SString& strMessage)
{
    ...
    if (auto validHandle = std::optional{hInstance ? hInstance : GetModuleHandle(nullptr)}; !validHandle || !*validHandle) [[unlikely]]
    {
        try
        {
            MessageBoxA(nullptr, "New York Chronicles has crashed."sv.data(), "New York Chronicles"sv.data(), MB_OK | MB_ICONERROR | MB_TOPMOST);
        }
        catch (...)
        {
        }
    ...
    struct ReentranceGuard final
    {
        ...
        explicit ReentranceGuard(std::atomic<bool>& f) noexcept : flag(f), isValid(!flag.exchange(true, std::memory_order_acquire)) {}
    ...
    try
    {
        ...
        if (!guard) [[unlikely]]
        {
            try
            {
                MessageBoxA(nullptr, "New York Chronicles has crashed."sv.data(), "New York Chronicles"sv.data(), MB_OK | MB_ICONERROR | MB_TOPMOST | MB_SETFOREGROUND);
            }
            catch (...)
            {
            }
        ...
        if (!hwndCrashedDialog)
        {
            ...
            if (!hwndCrashedDialog) [[unlikely]]
            {
                ...
                [[nodiscard]] constexpr auto cleanup = []() noexcept -> void
                {
                    ...
                    try
                    {
                        MessageBoxA(nullptr, CRASH_MSG_DIALOG_FAIL.data(), CRASH_TITLE_FATAL.data(), MB_OK | MB_ICONERROR | MB_TOPMOST);
                    }
                    catch (...)
                    {
                    }
            ...
            if (auto hwndCrashInfo = GetDlgItem(hwndCrashedDialog, IDC_CRASH_INFO_EDIT); hwndCrashInfo)
            {
                try
                {
                    ...
                    {
                        if (const auto wszMessage = FromUTF8(strSanitizedMessage); wszMessage && (wszMessage[0] != L'\0')) [[likely]]
                        {
        ...
        [[nodiscard]] constexpr auto calculateElapsed = [](const DWORD current, const DWORD start) noexcept -> DWORD
        {
            ...
            {
                return (current >= start) ? (current - start) : (std::numeric_limits<DWORD>::max() - start + current + 1);
            }
            else
            {
        ...
        for (;;)
        {
            const auto [cancelPressed, okPressed] = std::pair{bCancelPressed.load(std::memory_order_acquire), bOkPressed.load(std::memory_order_acquire)};
            if (cancelPressed || okPressed)
                break;

            if (const auto dwCurrentTime = static_cast<DWORD>(GetTickCount32()); calculateElapsed(dwCurrentTime, dwStartTime) > MAX_WAIT_TIME) [[unlikely]]
            {
...
{
    static constexpr const char* fallbackMessage = "The crash you experienced is due to memory usage. Please restart the game.";
    static constexpr const char* fallbackTitle = "New York Chronicles - Out of Memory";

    const char* message =
        _("The crash you experienced is due to memory usage.\n\n"
          "This is a x86 game with address space limits up to 3.6GB. "
          "Some servers with many mods can cause this.\n\n"
          "Please restart the game.");

    const char* title = _("New York Chronicles - Out of Memory Information");

    MessageBoxA(NULL, message ? message : fallbackMessage, title ? title : fallbackTitle, MB_OK | MB_ICONINFORMATION | MB_TOPMOST | MB_SETFOREGROUND);
}
...
void TestDialogs()
{
    ...
    ShowProgressDialog( g_hInstance, "New York Chronicles" );
    for ( uint i = 0 ; i < 100 ; i++ )
    {
