...
bool CConnectManager::Connect(const char* szHost, unsigned short usPort, const char* szNick, const char* szPassword, bool bNotifyServerBrowser)
{
    ...
    // Parse host into a server item (handles DNS resolution)
    ...
    {
        CCore::GetSingleton().ShowMessageBox(_("Error") + _E("CC22"), "Connection to New York Chronicles failed!", MB_BUTTON_OK | MB_ICON_ERROR);
    ...
    SString strBuffer = m_bReconnect ? "Reconnecting to New York Chronicles..." : "Connecting to New York Chronicles...";
    CGraphics::GetSingleton().SetStatusMessage(strBuffer);
