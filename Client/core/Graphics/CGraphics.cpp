...
{
    if (m_bProgressVisible || m_bDownloadVisible || m_bStatusVisible)
    {
        if (m_bProgressVisible && m_FirstDrawnProgressTimer.Get() > DUMMY_PROGRESS_MIN_DISPLAY_TIME)
...
////////////////////////////////////////////////////////////////
//
void CGraphics::SetStatusMessage(const SString& strMessage)
{
    m_strStatusMessage = strMessage;
    m_bStatusVisible = !strMessage.empty();
    if (m_bStatusVisible)
        m_StatusExpireTimer.Reset();
}

void CGraphics::ClearStatusMessage()
{
    m_strStatusMessage.clear();
    m_bStatusVisible = false;
}

void CGraphics::SetDownloadProgress(const SString& strMessage, float fProgress)
{
    m_strDownloadMessage = strMessage;
    m_fDownloadProgress = Clamp(0.f, fProgress, 1.f);
    m_bDownloadVisible = true;
}

void CGraphics::ClearDownloadProgress()
{
    m_strDownloadMessage.clear();
    m_fDownloadProgress = 0.f;
    m_bDownloadVisible = false;
}
...
void CGraphics::DrawProgressMessage(bool bPreserveBackbuffer)
{
    ...
    do
    {
...

        {
            ...
            const uint uiVH = GetViewportHeight();
            const uint uiVW = GetViewportWidth();
            const uint uiMargin = 24;
            const uint uiPadX = 20;
            const uint uiPadY = 14;
            const uint uiBarW = 4;
            const float fScale = 1.5f;
            ID3DXFont* pFont = GetFont(FONT_DEFAULT_BOLD);
            float fBottomY = (float)(uiVH - uiMargin);

            auto drawNotification = [&](const SString& text) {
                const uint tw = (uint)GetDXTextExtent(text, fScale, pFont);
                const uint bw = tw + uiPadX * 2 + uiBarW;
                const uint textH = (uint)GetDXFontHeight(fScale, pFont);
                const uint bh = textH + uiPadY * 2;
                const float fx = (float)(uiVW - bw - uiMargin);
                const float fy = fBottomY - (float)bh;
                DrawRectangleInternal(fx, fy, (float)bw, (float)bh, 0xE0101014, false);
                DrawRectangleInternal(fx, fy, (float)uiBarW, (float)bh, 0xFFB42828, false);
                int tx = (int)(fx + uiBarW + uiPadX);
                int ty = (int)(fy + uiPadY);
                DrawString(tx, ty, tx, ty, 0xDDFFFFFF, text, fScale, fScale, DT_NOCLIP, pFont);
                fBottomY = fy - 10;
            };

            if (m_bDownloadVisible && !m_strDownloadMessage.empty())
            {
                const uint tw = (uint)GetDXTextExtent(m_strDownloadMessage, fScale, pFont);
                const uint minW = 320;
                const uint bw = std::max(minW, tw + uiPadX * 2 + uiBarW);
                const uint textH = (uint)GetDXFontHeight(fScale, pFont);
                const uint progH = 4;
                const uint bh = textH + uiPadY * 2 + progH + 8;
                const float fx = (float)(uiVW - bw - uiMargin);
                const float fy = fBottomY - (float)bh;
                ...
                int tx = (int)(fx + uiBarW + uiPadX);
                int ty = (int)(fy + uiPadY);
                DrawString(tx, ty, tx, ty, 0xDDFFFFFF, m_strDownloadMessage, fScale, fScale, DT_NOCLIP, pFont);

                float barX = fx + uiBarW + uiPadX;
                float barY = fy + textH + uiPadY + 6;
                float barW = (float)(bw - uiBarW - uiPadX * 2);
                DrawRectangleInternal(barX, barY, barW, (float)progH, 0x40FFFFFF, false);
                DrawRectangleInternal(barX, barY, barW * m_fDownloadProgress, (float)progH, 0xFFB42828, false);

                fBottomY = fy - 10;
            }

            if (m_bStatusVisible && !m_strStatusMessage.empty())
            {
                if (m_StatusExpireTimer.Get() >= m_ulStatusLifeMs)
                    ClearStatusMessage();
                else
                    drawNotification(m_strStatusMessage);
            }

            if (!m_strProgressMessage.empty())
                drawNotification(m_strProgressMessage);
