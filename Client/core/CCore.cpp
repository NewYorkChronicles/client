...
{
    RemoveMessageBox();
    CGraphics::GetSingleton().SetStatusMessage(szText);
}
...
{
    CGraphics::GetSingleton().SetStatusMessage(strMessage);
}
...
bool CCore::CheckDiskSpace(uint uiResourcesPathMinMB, uint uiDataPathMinMB)
{
    ...
    {
        SString strMessage(_("New York Chronicles cannot continue because drive %s does not have enough space."), *strDriveWithNoSpace);
...
void CCore::DoPostFramePulse()
{
    ...
    if (m_pGame->GetSystemState() == SystemState::GS_FRONTEND)
    {
        ...
        if (m_menuFrame >= 5 && !m_isNetworkReady && m_pNet->IsReady())
        {
            ...
            // If launched with nyc:// URI containing newyorkchronicles, parse host:port from it
            // e.g. nyc://editor.newyorkchronicles.online:22010
            bool bConnectedFromURI = false;
            if (m_szCommandLineArgs && strnicmp(m_szCommandLineArgs, "nyc://", 6) == 0)
            {
                SString strURI = m_szCommandLineArgs;
                if (strURI.ContainsI("newyorkchronicles"))
                {
                    SString strHostPort = strURI.substr(6); // skip "nyc://"
                    // Remove trailing slash or whitespace
                    strHostPort = strHostPort.TrimEnd("/").TrimEnd(" ");
                    SString strHost, strPort;
                    if (strHostPort.Split(":", &strHost, &strPort))
                    {
                        CCommandFuncs::Connect(SString("%s %s", strHost.c_str(), strPort.c_str()));
                    }
                    else
                    {
                        CCommandFuncs::Connect(SString("%s 22003", strHostPort.c_str()));
                    }
                    bConnectedFromURI = true;
                }
            }

            if (!bConnectedFromURI)
                CCommandFuncs::Connect("play.newyorkchronicles.online 22003");
        }

        if (m_menuFrame >= 75 && m_requestNewNickname)
        {
...
void CCore::CalculateStreamingMemoryRange()
{
    ...
    m_fMinStreamingMemory = fMin;
    m_fMaxStreamingMemory = fMax;
}
