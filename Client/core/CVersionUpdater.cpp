...
{
    SString strURL("nyc://%s:%s@%s", *m_strSidegradeName, *m_strSidegradePassword, *m_strSidegradeHost);
...
{
    GetQuestionBox().Reset();
    GetQuestionBox().SetTitle(SString(_("New York Chronicles %s required"), *m_strSidegradeVersion));
    GetQuestionBox().SetMessage(SString(_("An updated version of New York Chronicles %s is required to join the selected server.\n\n"
                                          "Do you want to download and install New York Chronicles %s ?"),
...
void CVersionUpdater::_DialogSidegradeLaunchQuestion()
{
    GetQuestionBox().Reset();
    GetQuestionBox().SetTitle(SString(_("New York Chronicles %s required"), *m_strSidegradeVersion));
    GetQuestionBox().SetMessage(SString(_("Do you want to launch New York Chronicles %s and connect to this server ?"), *m_strSidegradeVersion));
    GetQuestionBox().SetButton(0, _("No"));
    GetQuestionBox().SetButton(1, _("Yes"));
    GetQuestionBox().Show();
    _PollQuestionNoYes();
...
void CVersionUpdater::_DialogDataFilesQuestion()
{
    ...
    GetQuestionBox().SetMessage(_("Some New York Chronicles data files are missing.\n\n\nPlease reinstall New York Chronicles"));
    GetQuestionBox().SetButton(0, _("Ok"));
    GetQuestionBox().Show();
    _PollAnyButton();
}
