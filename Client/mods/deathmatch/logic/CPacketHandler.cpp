...
void CPacketHandler::Packet_ServerConnected(NetBitStreamInterface& bitStream)
{
    ...
    // Connected silently — no chatbox message
...
void CPacketHandler::Packet_ServerDisconnected(NetBitStreamInterface& bitStream)
{
    ...
    if (bShowMessageBox)
    {
        ...
        std::vector<const char*> snippetList = {
            ...
            {_td("Ensure no other program is modifying New York Chronicles")},
        };
            ...
            bitStream.ReadCompressed(usDimension) && bitStream.ReadBit(bIsAttached))
        {
            ...
            switch (ucEntityTypeID)
            {
                ...
                case CClientGame::VEHICLE:
                {
                    ...
                    // Read vehicle model as unsigned short (supports stock 400-611 and custom 27000-27284)
                    unsigned short usModel = 0;
                    bitStream.Read(usModel);
                ...
                case CClientGame::PATH_NODE:
                {
                    ...
                    if (!(bitStream.Read(vecPosition.fX) && bitStream.Read(vecPosition.fY) && bitStream.Read(vecPosition.fZ) &&
                          bitStream.Read(vecRotation.fX) && bitStream.Read(vecRotation.fY) && bitStream.Read(vecRotation.fZ) && bitStream.Read(iTime) &&
                          bitStream.Read(ucStyle) && bitStream.Read(NextNodeID)))
                    {
