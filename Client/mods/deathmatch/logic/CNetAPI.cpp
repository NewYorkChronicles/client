...
void CNetAPI::WriteVehiclePuresync(CClientPed* pPlayerModel, CClientVehicle* pVehicle, NetBitStreamInterface& BitStream)
{
    ...
    // Use a stock model ID for non-standard vehicle model IDs.
    // This avoids a mismatch between client and server, ensuring doors and damage sync correctly.
    int iModelID = pVehicle->GetModel();
    if (iModelID < 400 || iModelID > 611)
    {
        // Service vehicles use behavior parent, others fall back to type-based stock default
        const auto& behaviorMap = GetCustomBehaviorMap();
        auto it = behaviorMap.find(static_cast<std::uint32_t>(iModelID));
        if (it != behaviorMap.end())
            iModelID = it->second;
        else
        {
            std::uint8_t vtype = GetCustomVehicleType(static_cast<std::uint32_t>(iModelID));
            switch (vtype)
            {
                case 0:  iModelID = 400; break;  // car
                case 1:  iModelID = 444; break;  // mtruck
                case 2:  iModelID = 471; break;  // quad
                case 3:  iModelID = 417; break;  // heli
                case 4:  iModelID = 460; break;  // plane
                case 5:  iModelID = 453; break;  // boat
                case 6:  iModelID = 537; break;  // train
                case 9:  iModelID = 461; break;  // bike
                case 10: iModelID = 481; break;  // bmx
                case 11: iModelID = 435; break;  // trailer
                default: iModelID = 400; break;
            }
        }
    }
