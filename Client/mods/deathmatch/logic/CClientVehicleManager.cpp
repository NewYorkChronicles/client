...
{
    std::uint8_t customType = GetCustomVehicleType(static_cast<std::uint32_t>(ulModel));
    if (customType != 0xFF)
    {
        switch (customType)
        {
            case 0:  return CLIENTVEHICLE_CAR;
            case 1:  return CLIENTVEHICLE_MONSTERTRUCK;
            case 2:  return CLIENTVEHICLE_QUADBIKE;
            case 3:  return CLIENTVEHICLE_HELI;
            case 4:  return CLIENTVEHICLE_PLANE;
            case 5:  return CLIENTVEHICLE_BOAT;
            case 6:  return CLIENTVEHICLE_TRAIN;
            case 9:  return CLIENTVEHICLE_BIKE;
            case 10: return CLIENTVEHICLE_BMX;
            case 11: return CLIENTVEHICLE_TRAILER;
        }
    }
...
unsigned char CClientVehicleManager::GetMaxPassengerCount(unsigned long ulModel)
{
    ...
    // Custom vehicles: try behavior parent first (only service vehicles have parents)
    if (ulModel >= CUSTOM_VEHICLE_MIN && ulModel <= CUSTOM_VEHICLE_MAX)
    {
        const auto& map = GetCustomBehaviorMap();
        auto it = map.find(static_cast<std::uint32_t>(ulModel));
        if (it != map.end() && it->second >= 400 && it->second <= 611)
            return g_ucMaxPassengers[it->second - 400];

        // No parent: default by vehicle type
        eClientVehicleType type = GetVehicleType(ulModel);
        switch (type)
        {
            case CLIENTVEHICLE_BIKE:
            case CLIENTVEHICLE_BMX:      return 1;
            case CLIENTVEHICLE_BOAT:     return 0;
            case CLIENTVEHICLE_QUADBIKE: return 1;
            default:                     return 3;
        }
    }
...
// Resolve custom model to stock parent for attribute lookups
static unsigned long ResolveAttributeModel(unsigned long ulModel)
{
    if (ulModel >= 400 && ulModel <= 611)
        return ulModel;
    if (ulModel >= CUSTOM_VEHICLE_MIN && ulModel <= CUSTOM_VEHICLE_MAX)
...
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_TURRENT));
}

bool CClientVehicleManager::HasSirens(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_SIRENS));
}

bool CClientVehicleManager::HasTaxiLight(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_TAXI_LIGHTS));
}

bool CClientVehicleManager::HasSearchLight(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_SEARCH_LIGHT));
}

bool CClientVehicleManager::HasLandingGears(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_LANDING_GEARS));
}

bool CClientVehicleManager::HasAdjustableProperty(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_ADJUSTABLE_PROPERTY));
}

bool CClientVehicleManager::HasSmokeTrail(unsigned long ulModel)
{
    unsigned long resolved = ResolveAttributeModel(ulModel);
    return (resolved && (g_ulVehicleAttributes[resolved - 400] & VEHICLE_HAS_SMOKE_TRAIL));
}
