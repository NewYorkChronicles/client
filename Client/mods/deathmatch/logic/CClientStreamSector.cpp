...
void CClientStreamSector::AddElements(list<CClientStreamElement*>* pList)
{
    ...
    std::unordered_set<CClientStreamElement*> existing;
    existing.reserve(pList->size());
    for (CClientStreamElement* p : *pList)
        existing.insert(p);

    for (CClientStreamElement* p : m_Elements)
    {
        if (existing.insert(p).second)
            pList->push_back(p);
    }
}
...
{
    if (m_Elements.empty() || pList->empty())
        return;

    std::unordered_set<CClientStreamElement*> toRemove(m_Elements.begin(), m_Elements.end());
    pList->remove_if([&toRemove](CClientStreamElement* p) { return toRemove.count(p) > 0; });
}
