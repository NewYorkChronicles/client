...
// Resolve a custom FLA model to a stock model for MTA lookups (handling, audio, sync).
// Service vehicles use the behavior parent map; others fall back to a stock default by type.
static std::uint16_t ResolveStockModel(unsigned short usModel)
{
    if (usModel >= 400 && usModel <= 611)
        return usModel;

    if (usModel >= CUSTOM_VEHICLE_MIN && usModel <= CUSTOM_VEHICLE_MAX)
    {
        // Service vehicles have an explicit parent (cop car -> copcarla, etc.)
        const auto& map = GetCustomBehaviorMap();
        auto it = map.find(static_cast<std::uint32_t>(usModel));
        if (it != map.end() && it->second >= 400 && it->second <= 611)
            return static_cast<std::uint16_t>(it->second);

        // Other custom vehicles: pick a stock default by vehicle type
        std::uint8_t vtype = GetCustomVehicleType(static_cast<std::uint32_t>(usModel));
        switch (vtype)
        {
            case 0:  return 400;  // car -> Landstalker
            case 1:  return 444;  // mtruck -> Monster
            case 2:  return 471;  // quad -> Quadbike
            case 3:  return 417;  // heli -> Leviathan
            case 4:  return 460;  // plane -> Skimmer
            case 5:  return 453;  // boat -> Jetmax (actually Reefer=453)
            case 6:  return 537;  // train -> Freight
            case 9:  return 461;  // bike -> PCJ-600
            case 10: return 481;  // bmx -> BMX
            case 11: return 435;  // trailer -> Trailer 1
        }
    }

    return 400;  // ultimate fallback
}
    ...
    : ClassInit(this), CClientStreamElement(pManager->GetVehicleStreamer(), ID)
{
    ...
    m_pOriginalHandlingEntry = g_pGame->GetHandlingManager()->GetOriginalHandlingData(m_usModel);
...
void CClientVehicle::SetModelBlocking(unsigned short usModel, unsigned char ucVariant, unsigned char ucVariant2)
{
    ...
    if (m_usModel != usModel)
    {
        ...
        if (IsLocalEntity() || (usModel >= 400 && usModel <= 611) || (usModel >= CUSTOM_VEHICLE_MIN && usModel <= CUSTOM_VEHICLE_MAX))
        {
            std::uint16_t usHandlingModelID = ResolveStockModel(usModel);
...
void CClientVehicle::Create()
{
    ...
    if (!m_pVehicle)
    {
        ...
        {
            std::uint32_t modelId = ResolveStockModel(m_usModel);
