...
{
    if (uiModelID >= static_cast<uint>(g_pGame->GetBaseIDforTXD()))
        throw std::invalid_argument(SString("Expected modelid in range 0 - %d, got %d", g_pGame->GetBaseIDforTXD() - 1, uiModelID));
...
{
    if (uiModelID >= static_cast<uint>(g_pGame->GetBaseIDforTXD()))
        throw std::invalid_argument(SString("Expected model ID in range [0-%d] at argument 1", g_pGame->GetBaseIDforTXD() - 1));
...
{
    CModelInfo* pModelInfo = g_pGame->GetModelInfo(uiModelId);
    const uint baseTxdId = static_cast<uint>(g_pGame->GetBaseIDforTXD());

    if (uiModelId >= baseTxdId || !pModelInfo)
        throw std::invalid_argument(SString("Expected a valid model ID in range [0-%d] at argument 1", baseTxdId - 1));
