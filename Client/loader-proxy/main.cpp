...
VOID OnGameLaunch()
{
    ...
    // Pre-core plugin modules — loaded in order before MTA core.
    // Add new entries here: { filename, display name }
    {
        struct NycModule { const wchar_t* filename; const char* label; };

        static constexpr NycModule kModules[] = {
            { L"limits.nyc",  "Limit Adjuster" },
            { L"starryskies.nyc",  "Starry Skies GTA SA" }
        };

        wchar_t savedCwd[MAX_PATH]{};
        GetCurrentDirectoryW(MAX_PATH, savedCwd);
        SetCurrentDirectoryW(gtaDirectory.wstring().c_str());
        SetDllDirectoryW(gtaDirectory.wstring().c_str());

        for (const auto& mod : kModules)
        {
            const fs::path modPath = gtaDirectory / mod.filename;
            if (!fs::is_regular_file(modPath, ec))
                continue;

            HMODULE hMod = LoadLibraryW(modPath.wstring().c_str());
            if (hMod)
            {
                AddLaunchLog("loader-proxy: Loaded %s from %S", mod.label, modPath.wstring().c_str());
            }
            else
            {
                DWORD err = GetLastError();
                AddLaunchLog("loader-proxy: Failed to load %s (%d): %s", mod.label, err, std::system_category().message(err).c_str());
            }
        }
    ...
    if (!g_core)
    {
        ...
        DisplayErrorMessageBox(
            ...
            L"\n\n"
            L"Please ensure that your New York Chronicles build was successful and that "
        ...
        DisplayErrorMessageBox(
            ...
            L"\n\n"
            L"Please ensure that your New York Chronicles installation is complete "
...
{
    std::wstring caption = L"New York Chronicles  [" + errorCode + L"]   (CTRL+C to copy)";
...
auto MakeLauncherError(std::wstring message) -> std::wstring
{
    ...
    message +=
        L"Please launch the game through the New York Chronicles launcher. "
        L"You can find the launcher either on your desktop or in the installation directory.";
...
auto MakeMissingFilesError(std::wstring message) -> std::wstring
{
    ...
    message +=
        L"Please ensure that both your New York Chronicles and Windows installation are not missing files "
