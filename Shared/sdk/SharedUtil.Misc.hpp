                    ...
                    const auto processPath = [&strProcessBaseDir](const std::wstring& fullPathStr)
                    {
                        ...
                        if (lastSeparator != std::wstring::npos)
                        {
                            ...
                            for (int level = 0; level < 3; ++level)
                            {
                                ...
                                std::wstring folderName = (lastSep != std::wstring::npos) ? currentPath.substr(lastSep + 1) : currentPath;
...
CDuplicateLineFilter<SReportLine> ms_ReportLineFilter;
    ...
    #define PRODUCT_REGISTRY_PATH   "Software\\New York Chronicles"  // HKLM
    #define PRODUCT_COMMON_DATA_DIR "New York Chronicles"          // C:\ProgramData
...
SString SharedUtil::GetMTASABaseDir()
{
    ...
    if (strInstallRoot.empty())
    {
        ...
        if (strInstallRoot.empty())
        {
            ...
            if (strInstallRoot.empty())
            {
                MessageBoxUTF8(0, _("New York Chronicles has not been installed properly, please reinstall."), _("Error") + _E("U01"),
...
bool SharedUtil::ProcessPendingBrowseToSolution()
{
    ...
    SString strTitle;
    strTitle.Format("New York Chronicles %s   (CTRL+C to copy)", strErrorCode.c_str());
